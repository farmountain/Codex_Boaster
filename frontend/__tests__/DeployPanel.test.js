...
global.fetch = jest.fn(() =>
  Promise.resolve({ json: () => Promise.resolve({ status: 'success', deployment_url: 'url' }) })
);
...
test('renders deploy form and submits request', async () => {
  ...
  fireEvent.change(screen.getByPlaceholderText('Repository URL'), { target: { value: 'http://repo' } });
